    ...
    return(
        ...
        <PostHeader>
            ...
            <div style={{fontWeight:'bold', fontSize:'14px'}}>
                {postInfo.user.nick} {/* 글쓴이 닉네임 */}
                </div>
            <div style={{color:'#7D7D7D', fontSize:'11px'}}>
                {postInfo.user.address} {/* 글쓴이 주소 */}
                </div>
            </div>
            <HighLight>{postInfo.startDate} - {postInfo.endDate}</HighLight> {/* 대여기간*/}
        </PostHeader>
        <PostTitle>
        ...
        <PostFooter>
            <HighLight>{postInfo.price}</HighLight> {/* 제품 가격*/}
            <ChatButton>채팅 보내기</ChatButton>
        </PostFooter>
        </PostComponent>
        </>
    );
