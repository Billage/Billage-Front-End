...
const MainBoards=()=>{
    ...
    const getLendYou=async (loginChk) => {
        if(loginChk){ //로그인이 되어있는 상태에서는 사용자와 같은 동 내의 글만 보여줍니다.
    ...
    useEffect(() => {  //처음 페이지가 실행될 때, 로그인 상태바뀔때, 사용자 주소 바뀔때 실행됨
    if(!isLogined){ //로그인 하지 않은 상황에서는 모든 글이 보여집니다
    ...
    .then(axios.spread(({lendResp}, {borrowResp}) => { // response를 spread로 받는다. 
        ...
        setLendYouList(lendResp.data.data);
        console.log(lendResp);
        setBorrowMeList(borrowResp.data.data);
        console.log(borrowResp)
    })).catch((error) => {
        console.error(error)
    })}
    else if(isLogined){//로그인 한 상황에서는 사용자와 같은 동 내의 게시물만 보여집니다
